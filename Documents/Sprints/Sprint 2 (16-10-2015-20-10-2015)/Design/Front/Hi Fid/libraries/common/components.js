...
prx.types.shape = {
	...
	,onDisplay: function(item,containerid) {
		...
		if(item.typeName == "oval" || item.typeName == "rectangle") {
			return prx.types.rectangle.onDisplay(item,containerid);
		}
		...
		if(item.typeName == 'polygon' || item.typeName == 'star') {
			path = prx.componentsHelper.drawComplexShape( item, item.typeName, dims.width, dims.height, item.borderWidthShape);
		}
		else {
			path = prx.componentsHelper.drawShape( item, item.typeName, dims.width, dims.height, item.borderWidthShape);
		}
	...
	,onResize: function(item,containerid) {
		...
		if(item.typeName == 'polygon' || item.typeName == 'star') {
			document.getElementById('path-'+ _id).setAttribute('d', prx.componentsHelper.drawComplexShape( item, item.typeName, dims.width, dims.height, item.borderWidthShape));
		} else {
			document.getElementById('path-'+ _id).setAttribute('d', prx.componentsHelper.drawShape( item, item.typeName, dims.width, dims.height, item.borderWidthShape));
		}
	}
	,interactions: [
...
prx.types.symbol = {
	...
	,onDisplay: function(item,containerid) {
		...
		if (_symbolindex > -1) {
			...
			var osWatchStyle = (item.scroll == 'vertical' && typeof(item.watchOSscrollbars)!== 'undefined' && item.watchOSscrollbars == true) ? true : false;
			/* /ADDED THESE LINES IN ORDER TO MAKE SYMBOLS BEHAVE RESPONSIVELY */

			if(typeof(item.propagateevents) == "undefined") { item.propagateevents = false; }


			cReturn = '<div id="' + _id + '" class="box type-symbol '+((item.propagateevents && (item.scroll == '' || item.scroll == 'none')) ? 'type-symbol-transparent' : '')+'" data-symbol-id="'+item.symbolid+'" '+dataState+' style="position: absolute;background-color: '+prx.utils.getColor(_bg)+';">';


			var applewatchScrollbarClass = '';
			if(isAppleWatchCrown == true && osWatchStyle==true){
			...
			cReturn +='<div id="'+_id+'-inner" class="'+applewatchScrollbarClass+'" style="position: absolute; overflow: hidden !important; height: 100%; width: 100%;"><div id="' + _id + '-scroll" class="symbol-scroll" style="width: '+_symbol_width+'; height: '+_symbol_height+'; position: absolute; overflow: hidden;">' + prx.stc.containers.render(item,containerid,stateindex) + '</div></div></div>';
		} else {
			_appenditem = false;
			if(prx.editor) {
			...
			properties: [[
              		{
      					...
      					,onChange: function(item){
	      					...
	      					if(item.scroll == '' || item.scroll == 'none') {
	      						$('#property-scrollsnap, #property-pinchzoom, #property-momentum, #property-scrollbars, #property-lockdirection, #property-bounce, #property-watchOSscrollbars').hide();
...
prx.types.rectangle = {
	...
	,onDisplay: function(item,containerid) {
		...
		var cR = '';
		cR += '<div id="' + _id + '" class="box pos type-rectangle" data-shape-type="'+item.typeName+'">';
		cR += '<style>';
	...
	, propertyGroups: [
		{
			...
			properties: [
				...
				[
					...
					values: [{
						value: 'left',
						displayValue: '<span class="property-icon property-align-left" title="Align left"></span>'
					}, {
						value: 'center',
						displayValue: '<span class="property-icon property-align-center" title="Align center"></span>'
					}, {
						value: 'right',
						displayValue: '<span class="property-icon property-align-right" title="Align right"></span>'
					}],
					changeProperty: {
						caption: 'Text Align',
						selector: '.liveUpdate-textColor',
...
prx.types.image = {
    ...
    , onDisplay: function (item, containerid) {
        ...
        if (prx.editor && item.imgSrc.fileId == "d310bece0e91b91b485ed62166d1fc2e.svg" && item.imgSrc.assetType == "gallery") {
            ...
            cR += '<style>';
            cR += '#'+_id+', #'+_id+' .type-image-wrapper { border-radius: '+item.borderRadius+'px;}';
    		cR += '#'+_id+' .type-image-wrapper { border: '+item.borderWidth+'px solid '+prx.utils.getColor(item.borderColor)+';  }';
    		cR += '</style>'
        }
        cR += '</div>';
...
prx.types.webview = {
	...
	,onDisplay: function(item,containerid) {
		...
		cR += '<iframe src='+prx.componentsHelper.getProp(item.url,'url')+' scrolling='+((item.scrollable) ? '"yes"' : '"no"') +' style="border: none; width: 100%; height: 100%;"></iframe>';
		if(!prx.editor && prx.iosmobilebrowser) {
		cR += '</div>';
		}
		cR += '</div>';
...
prx.types.audio = {
	name: 'audio'
		...
		cR += '<audio '+ ((item.controls) ? 'controls' : '') +' '+ ((item.preload) ? 'preload' : '') +' '+ ((item.autoplay) ? 'autoplay' : '') +' '+ ((item.loop) ? 'loop' : '') +'>';
...
prx.types.video = {
	...
	,onDisplay: function(item,containerid) {
		...
		case 'html5':
			cR += '<video id="'+_id+'-html5video" class="html5" width="100%" height="100%" '+ ((item.controls) ? 'controls' : '') + ((item.placeholder.fileId != '') ? ' poster='+prx.componentsHelper.getAssetUrl(item.placeholder) : '' )+' '+ ((!item.preload || prx.editor) ? 'preload="none"' : '') +' '+ ((item.autoplay) ? 'autoplay' : '') +' '+((item.loop) ? 'loop' : '')+' webkit-playsinline>';
		...
		case 'youtube':
			if(prx.editor && $.browser.chrome) {
				...
				cR += '<iframe id="'+_id+'-youtube"   width="100%" height="100%" src="https://www.youtube.com/embed/'+item.youtubeid+'?enablejsapi=1&html5=1&wmode=transparent&playsinline=1'+((item.autoplay && !prx.editor) ? '&autoplay=1' : '')+((item.loop && !prx.editor) ? '&loop=1&playlist='+item.youtubeid : '')+((!item.controls) ? '&controls=0' : '')+'" frameborder="0"  webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
			}
			break;
		case 'vimeo':
			if(prx.editor && $.browser.chrome) {
				...
				cR += '<iframe id="'+_id+'-vimeo"  width="100%" height="100%" src="https://player.vimeo.com/video/'+item.vimeoid+'?title=0&amp;byline=0&amp;portrait=0&wmode=transparent'+((item.autoplay && !prx.editor) ? '&autoplay=1' : '')+((item.loop && !prx.editor) ? '&loop=1' : '')+'" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
			}
			break;
		}
		cR += '</div>';
...
prx.types.tooltip = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		case 'right':
			...
			_pos = 'width: '+((12*prx.componentsHelper.getScale(item.lib)) + parseInt(item.borderWidth))+'px; height: '+(25*prx.componentsHelper.getScale(item.lib))+'px; top: '+eval((_dims.height * item.ttPosition/100) - ((25*prx.componentsHelper.getScale(item.lib)) * item.ttPosition/100))+'px;';
			break;
		case 'none':
			_height = '100%';
			_width = '100%';
